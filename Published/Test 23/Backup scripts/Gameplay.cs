...
public class Gameplay : NetworkBehaviour
{
    ...
    public void CmdStartGame(List<bool> _settings, List<int> randomCardFrequencies)
    {
        ...
        int firstcard = getCardValue(Random.Range(0,4), Random.Range(0,10));
        cardOnTable = firstcard;
        setCardOnTable(cardOnTable);
       ...
       /* int n = players.Count;
        ...
        for (int i = 0; i < players.Count; i++)
        {
            if (i == turn)
            {
                players[i].RpcTurnStart(true, 0);
            }
            else
            {
                players[i].RpcTurnStart(false, 0);
            }
        }
    ...
    //Syncing info methods
    [Server]
    public void incrementNeedToDraw(int amount)
    {
        needToDraw += amount;
        if (needToDraw < 0) needToDraw = 0;
        RpcSetNeedToDrawBuffer(needToDraw);

    }

    [Server]
    public void setNeedToDraw(int amount)
    {
        needToDraw = amount;
        if (needToDraw < 0) needToDraw = 0;
        RpcSetNeedToDrawBuffer(needToDraw);
    ...
    [Server]
    public void setCardOnTable( int value)
    {
        cardOnTable = value;
        RpcSetCardOnTableBuffer(cardOnTable);
    }
    ...
    //Turn methods
    [Server]
    public void PassTurnToNextPlayer(int drawCards = 0, int skip = 0, int reverse = 0, int cardsDrawn = 0, bool Iwin = false, string myname = "NOT FOUND")
    {
        if (Iwin)
        {
            RpcEndGame(myname);
        ...
        {
            needToDraw = 0;
            RpcSetNeedToDrawBuffer(0);
        }
        

        if ((getCardNumber(cardOnTable) != 10 && getCardNumber(cardOnTable) != 13))
        {
            drawCards = 0;
            needToDraw = 0;
            RpcSetNeedToDrawBuffer(0);
        ...
        Color temp = Sceneobjects.current.CardColours[getCardColour(cardOnTable)];
        ...
        for(int i = 0; i < players.Count; i++)
        {
            ...
            if(i == turn)
            {
                players[i].RpcTurnStart(true, needToDraw);
            }
            else
            {
                players[i].RpcTurnStart(false, needToDraw);
            }
        }
        //CmdUpdatePlayerNames();
    }
    ...
    {
        UIController.current.winText.text = winner + " is victorious!";
